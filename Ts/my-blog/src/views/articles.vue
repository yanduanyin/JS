      ...
      <transition-group name="el-fade-in">
        <li class="item" v-for="(item, index) in articlesList" :key="index">
          <a href="#" target="_blank">
            <img src="../assets/bg.jpg" alt="" class="wrap-img img-blur-done" :data-src="item.img_url" data-has-lazy-src="false">
            <div class="content">
...
// 懒加载
const viewHeight = window.innerHeight || document.documentElement.clientHeight; // 可视区域

const lazyload = () => { // 图片懒加载
  const imgs = document.querySelectorAll('#list .item img');
  let num = 0; // 统计显示到了哪一张图片
...
export default class Articles extends Vue {
  ...
  mounted (): void {
    this.handleSearch();
    document.addEventListener('scroll', lazyload) // 页面滚动也调用lazyload
  }
